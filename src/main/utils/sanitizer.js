...
function sanitizeString(str) {
  ...
  sanitized = sanitized.replace(/\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b/gi, '[JOB_ID_REDACTED]');

  // Pattern: Long numeric IDs (PlaceId, UserId - 10+ digits)
  sanitized = sanitized.replace(/\b\d{10,}\b/g, '[ID_REDACTED]');

  // Pattern: Cookie values
  sanitized = sanitized.replace(/\.(ROBLOSECURITY|ROBLOXID)=[^;]+/gi, '.$1=[REDACTED]');

  // Pattern: Access tokens in URLs or query strings
  sanitized = sanitized.replace(/([?&])(access_token|token|auth)=([^&\s]+)/gi, '$1$2=[REDACTED]');
