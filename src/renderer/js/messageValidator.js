...
class MessageValidator {
  ...
  checkSwearWords(message) {
    ...
    for (const swear of this.swearWords) {
      ...
      if (swear.length >= 3) {
        ...
        const middlePattern = middleLetters
          .split("")
        ...
        if (match && match[0].match(/[^a-z]/)) {
          ...
          const charBefore =
            matchIndex > 0 ? lowerMessage[matchIndex - 1] : " ";
          const charAfter =
            matchIndex + matchedText.length < lowerMessage.length
  ...
  checkSpam() {
    ...
    this.messageTimes = this.messageTimes.filter(
      (time) => now - time < this.SPAM_WINDOW_MS,
    );
    ...
    if (this.messageTimes.length >= this.MAX_MESSAGES_PER_MINUTE) {
      ...
      const waitTime = Math.ceil(
        (this.SPAM_WINDOW_MS - (now - oldestMessage)) / 1000,
      );
